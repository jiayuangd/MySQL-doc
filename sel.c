...
/*	fprintf(cgiOut, "<head><meta charset=\"utf-8\"/><title>查询结果</title>\
	...
	if (name[0] == '*')
	{
		sprintf(sql, "select score.stuId, name, age, dept, score.cno, grade from course,score,info where info.stuId=score.stuId and score.cno=course.cno and state!='0'");
	}
	else
	{
		sprintf(sql, "select score.stuId, name, age, dept, score.cno, grade from course,score,info where name = '%s' and info.stuId=score.stuId and score.cno=course.cno and state!='0'",name);
	}
